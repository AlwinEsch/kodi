...
#include "RendererDRMPRIMEVulkan.h"
...
#include "cores/VideoPlayer/VideoRenderers/VideoShaders/YUV2RGBShaderVulkan.h"
...
CBaseRenderer* CRendererDRMPRIMEVulkan::Create(CVideoBuffer* buffer)
{
  //if (!buffer)
    return nullptr;
...
//#endif
//
//  return new CRendererDRMPRIMEVulkan();
}

void CRendererDRMPRIMEVulkan::Register()
{
  VIDEOPLAYER::CRendererFactory::RegisterRenderer("drm_prime_vulkan",
                                                  CRendererDRMPRIMEVulkan::Create);
}

bool CRendererDRMPRIMEVulkan::Configure(const VideoPicture& picture,
                                        float fps,
...
//    CLog::Log(LOGWARNING, "RendererDRMPRIMEVulkan::Configure: SetVideoOutput failed");
//
...
//    auto shader = std::make_unique<Shaders::VULKAN::BaseYUV2RGBVulkanShader>(
...
//      CLog::Log(LOGINFO,
//                "RendererDRMPRIMEVulkan::Configure: limited-range YUV path "
//                "available (src bits {}, src {} range, fourcc {:#x})",
//                picture.colorBits, picture.color_range ? "full" : "limited", sourceFourcc);
//    }
//    else
//    {
//      CLog::Log(LOGWARNING, "RendererDRMPRIMEVulkan::Configure: limited-range "
...
//  CLog::Log(LOGDEBUG, "RendererDRMPRIMEVulkan::Configure: HDR passthrough: {}",
...
//    CLog::Log(LOGWARNING, "RendererDRMPRIMEVulkan::Configure: HDR passthrough active but GUI "
//                          "compositing not supported by windowing system");

  return true;
}

bool CRendererDRMPRIMEVulkan::IsGuiLayer()
{
  return !m_hdrFboActive;
}

void CRendererDRMPRIMEVulkan::UnInit()
{
  //Flush(false);
...
void CRendererDRMPRIMEVulkan::AddVideoPicture(const VideoPicture& picture, int index)
{
...
bool CRendererDRMPRIMEVulkan::Flush(bool saveBuffers)
{
  //if (!saveBuffers)
...
void CRendererDRMPRIMEVulkan::ReleaseBuffer(int index)
{
  //BUFFER& buf = m_buffers[index];
...
void CRendererDRMPRIMEVulkan::Update()
{
  //if (!m_configured)
  //  return;
...
void CRendererDRMPRIMEVulkan::DrawBlackBars()
{
...
void CRendererDRMPRIMEVulkan::RenderUpdate(
...
bool CRendererDRMPRIMEVulkan::RenderCapture(int index, CRenderCapture* capture)
{
  //capture->BeginRender();
  //capture->EndRender();
  return true;
...
bool CRendererDRMPRIMEVulkan::ConfigChanged(const VideoPicture& picture)
{
...
void CRendererDRMPRIMEVulkan::Render(unsigned int flags, int index)
{
  //BUFFER& buf = m_buffers[index];
  ...
  //  m_yuvShader->SetMatrices(vulkanMatrixProject.Get(), vulkanMatrixModview.Get());
  ...
  //renderSystem->EnableGUIShader(VulkanShaderMethod::SM_TEXTURE_RGBA_OES);
...
bool CRendererDRMPRIMEVulkan::Supports(ERENDERFEATURE feature) const
{
  switch (feature)
  {
...
bool CRendererDRMPRIMEVulkan::Supports(ESCALINGMETHOD method) const
{
  switch (method)
  {
