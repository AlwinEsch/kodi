...
#include "OverlayRendererVulkan.h"
...
// Vulkan2.0 cant do CLAMP, but can do CLAMP_TO_EDGE.
                        ...
                        const GLvoid* pixels)
{
  ...
  int bytesPerPixel = KODI::UTILS::VULKAN::vulkanFormatElementByteCount(externalFormat);

  bool bgraSupported = false;

  if (!alpha)
  {
    if (CVulkanExtensions::IsExtensionSupported(CVulkanExtensions::EXT_texture_format_BGRA8888) ||
    ...
    else if (CVulkanExtensions::IsExtensionSupported(CVulkanExtensions::APPLE_texture_format_BGRA8888))
    {
...
{
  return std::make_shared<COverlayTextureVulkan>(o, rSource);
}

COverlayTextureVulkan::COverlayTextureVulkan(const CDVDOverlayImage& o, CRect& rSource)
{
...
{
  return std::make_shared<COverlayTextureVulkan>(o);
}

COverlayTextureVulkan::COverlayTextureVulkan(const CDVDOverlaySpu& o)
{
...
{
  return std::make_shared<COverlayGlyphVulkan>(images, width, height);
}

COverlayGlyphVulkan::COverlayGlyphVulkan(ASS_Image* images, float width, float height)
{
  m_width = 1.0;
  m_height = 1.0;
...
COverlayGlyphVulkan::~COverlayGlyphVulkan()
{
  glDeleteTextures(1, &m_texture);
}

void COverlayGlyphVulkan::Render(SRenderState& state)
{
  ...
  vulkanMatrixModview->Translatef(state.x, state.y, 0.0f);
  vulkanMatrixModview->Scalef(state.width, state.height, 1.0f);
...
COverlayTextureVulkan::~COverlayTextureVulkan()
{
  glDeleteTextures(1, &m_texture);
}

void COverlayTextureVulkan::Render(SRenderState& state)
{
  glEnable(GL_BLEND);
  ...
  CVulkanRenderSystem* renderSystem =
      dynamic_cast<CVulkanRenderSystem*>(CServiceBroker::GetRenderSystem());
  renderSystem->EnableGUIShader(VulkanShaderMethod::SM_TEXTURE_NOBLEND);
  GLint posLoc = renderSystem->GUIShaderGetPos();
  GLint tex0Loc = renderSystem->GUIShaderGetCoord0();
  GLint depthLoc = renderSystem->GUIShaderGetDepth();
  ...
  // across the bitmap quad. CVulkanShader::OnEnabled resets this to 0.0 every
