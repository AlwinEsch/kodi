...
// --- CRendererFactoryVulkan ------------------------------------------------

std::string CRendererFactoryVulkan::RenderSystemName() const
{
  return "Vulkan";
}

CRPBaseRenderer* CRendererFactoryVulkan::CreateRenderer(
    const CRenderSettings& settings,
    CRenderContext& context,
    std::shared_ptr<IRenderBufferPool> bufferPool)
{
  return new CRPRendererVulkan(settings, context, std::move(bufferPool));
}

RenderBufferPoolVector CRendererFactoryVulkan::CreateBufferPools(CRenderContext& context)
{
  return {std::make_shared<CRenderBufferPoolVulkan>(context)};
}
...
bool CRPRendererVulkan::SupportsScalingMethod(SCALINGMETHOD method)
{
