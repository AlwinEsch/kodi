...
#include "ShaderTextureVulkanRef.h"
...
bool CShaderVulkan::Create(unsigned int passIdx,
                           ...
                           unsigned int frameCountMod)
{
  //if (shaderPath.empty())
  //{
  //  CLog::Log(LOGERROR, "CShaderVulkan::Create: Can't load empty shader path");
  //  return false;
  //}
  ...
  //std::string defineVersion = CShaderUtilsVulkan::GetGLSLVersion(m_shaderSource);
  ...
  //  CLog::Log(LOGERROR, "CShaderVulkan::Create: Vertex shader compile error:\n{}",
  ...
  //  CLog::Log(LOGERROR, "CShaderVulkan::Create: Fragment shader compile error:\n{}",
  //            std::string(errorLog.begin(), errorLog.end()));
  //}
  ...
  //  CLog::Log(LOGERROR, "CShaderVulkan::Create: Shader program link error:\n{}",
  //            std::string(errorLog.begin(), errorLog.end()));
  //  CLog::Log(LOGERROR, "CShaderVulkan::Create: Failed to load video shader: {}", m_shaderPath);
  //  return false;
  //}
...
void CShaderVulkan::Render(IShaderTexture& sourceTexture, IShaderTexture& targetTexture)
{
  //glDisable(GL_BLEND);
...
void CShaderVulkan::SetSizes(const float2& nextSize,
...
void CShaderVulkan::PrepareParameters(
...
void CShaderVulkan::UpdateUniformInputs(
    IShaderTexture& sourceTexture,
    const std::vector<std::unique_ptr<IShaderTexture>>& pShaderTextures,
    const std::vector<std::unique_ptr<IShader>>& pShaders,
    uint64_t frameCount)
{
  ...
  //{
  //  auto& sourceGL = static_cast<CShaderTextureVulkan&>(*pShaderTextures[m_passIdx - 1]);
  //  m_uniformFrameInputs = GetFrameInputData(sourceGL.GetTextureID());
  //}
  //else // First pass
  //{
  //  auto& sourceGLRef = static_cast<CShaderTextureVulkanRef&>(sourceTexture);
  ...
  //{
  //  const auto& shader = static_cast<const CShaderVulkan&>(*pShaders[i]);
  //  UniformFrameInputs frameInput = shader.GetFrameUniformInputs();
  //  m_passesUniformFrameInputs.emplace_back(frameInput);
  //}
}

CShaderVulkan::UniformInputs CShaderVulkan::GetInputData(uint64_t frameCount) const
{
...
void CShaderVulkan::GetUniformLocs()
{
...
void CShaderVulkan::SetShaderParameters(IShaderTexture& sourceTexture)
{
  //// Set shader uniforms
  ...
  //auto* sourceGL = dynamic_cast<CShaderTextureVulkan*>(&sourceTexture);
  //auto* sourceGLRef = dynamic_cast<CShaderTextureVulkanRef*>(&sourceTexture);
