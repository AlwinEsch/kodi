...
CShaderPresetVulkan::CShaderPresetVulkan(RETRO::CRenderContext& context,
...
bool CShaderPresetVulkan::CreateShaders()
{
  ...
  if (!m_passes.empty())
  {
    ...
    for (unsigned int i = 0; i < numLuts; ++i)
    {
      ...
      auto presetLut = std::make_shared<CShaderLutVulkan>(lutStruct.strId, lutStruct.path);
  ...
  for (unsigned int shaderIdx = 0; shaderIdx < numPasses; ++shaderIdx)
  {
    ...
    // Create the shader
    auto videoShader = std::make_unique<CShaderVulkan>();
    ...
    {
      CLog::Log(LOGERROR, "CShaderPresetVulkan::CreateShaders: Couldn't create a video shader");
      return false;
    }
...
void CShaderPresetVulkan::RenderShader(IShader& shader,
