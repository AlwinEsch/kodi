...
bool CWebGUIInfo::GetLabel(std::string& value,
                           const CFileItem* item,
                           int contextWindow,
                           const CGUIInfo& info,
                           std::string* fallback) const
{
  if (info.GetInfo() >= WEB_CONTROL_START && info.GetInfo() <= WEB_CONTROL_END)
  {
...
bool CWebGUIInfo::GetInt(int& value,
...
bool CWebGUIInfo::GetBool(bool& value,
                          const CGUIListItem* item,
                          int contextWindow,
                          const CGUIInfo& info) const
{
  if (info.GetInfo() >= WEB_CONTROL_START && info.GetInfo() <= WEB_CONTROL_END)
  {
    WEB::CGUIWindowWebBrowser* window =
        GetWebBrowserWindow(contextWindow); // true for has web control
    if (window)
    {
      switch (info.GetInfo())
      {
