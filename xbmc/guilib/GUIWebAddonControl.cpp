...
#include "addons/interfaces/gui/GUITranslator.h"
...
CGUIWebAddonControl::CGUIWebAddonControl(int parentID,
                                         int controlID,
                                         float posX,
                                         float posY,
                                         float width,
...
void CGUIWebAddonControl::UpdateInfo(const CGUIListItem* item)
{
  if (item && m_bInvalidated)
  {
    if (m_addonControl == nullptr)
    {
      ...
      {
        CLog::Log(LOGERROR,
                  "CGUIWebAddonControl::{}: Called without defined 'browseridstring' for '{}'",
                  __func__, item->GetLabel2());
        return;
      }
      ...
      if (m_addonControl == nullptr)
      {
        CLog::Log(LOGERROR, "CGUIWebAddonControl::{}: Failed to get web addon control!", __func__);
        return;
      }
...
bool CGUIWebAddonControl::OnMessage(CGUIMessage& message)
{
  if (m_addonControl && message.GetControlId() == GetID())
  {
    switch (message.GetMessage())
    {
      ...
      {
        CWebFavourites::AddToFavourites(m_addonControl->OpenedTitle(),
                                        m_addonControl->OpenedAddress(), m_addonControl->IconURL());
        return true;
      }
      default:
        break;
...
EVENT_RESULT CGUIWebAddonControl::OnMouseEvent(const CPoint& point,
                                               const KODI::MOUSE::CMouseEvent& event)
{
  if (m_addonControl && m_addonControl->OnMouseEvent(event.m_id, point.x, point.y, event.m_offsetX,
                                                     event.m_offsetY, event.m_state))
...
bool CGUIWebAddonControl::OnAction(const CAction& action)
{
  ...
  actionAddon.id = ADDON::CAddonGUITranslator::TranslateActionIdToAddon(action.GetID());
  ...
  if (!m_addonControl || !m_addonControl->OnAction(&actionAddon, nextItem))
...
void CGUIWebAddonControl::Process(unsigned int currentTime, CDirtyRegionList& dirtyregions)
{
  ...
  if (m_addonControl && !m_attemptedLoad)
  {
    ...
    auto& gfxContect = CServiceBroker::GetWinSystem()->GetGfxContext();

    gfxContect.CaptureStateBlock();
    float x = gfxContect.ScaleFinalXCoord(GetXPosition(),
                                                                               GetYPosition());
    float y = gfxContect.ScaleFinalYCoord(GetXPosition(),
                                                                               GetYPosition());
    float w = gfxContect.ScaleFinalXCoord(
                  GetXPosition() + m_props.fSkinWidth, GetYPosition() + m_props.fSkinHeight) -
              x;
    float h = gfxContect.ScaleFinalYCoord(
                  GetXPosition() + m_props.fSkinWidth, GetYPosition() + m_props.fSkinHeight) -
              y;
    if (x < 0)
      x = 0;
    if (y < 0)
      y = 0;
    if (x + w > gfxContect.GetWidth())
      w = gfxContect.GetWidth() - x;
    if (y + h > gfxContect.GetHeight())
      h = gfxContect.GetHeight() - y;

    strncpy(m_props.strName, m_idString.c_str(), sizeof(m_props.strName));
    ...
    m_props.fPixelRatio = gfxContect.GetResInfo().fPixelRatio;
    m_props.fFPS = gfxContect.GetFPS();
    ...
    std::string startAddress =
        GenerateAddress(m_loadAddress.empty() ? m_startAddress : m_loadAddress);
    if (m_addonControl->CreateWebControl(this, startAddress))
    {
      CLog::Log(LOGDEBUG, "{} - Web addon control successfully created, for '{}'", __func__,
                m_idString);
      gfxContect.ApplyStateBlock();
    }
    else
    {
      CLog::Log(LOGERROR, "{} - Web addon control creation for '{}' failed", __func__, m_idString);
...
void CGUIWebAddonControl::Render()
{
  ...
  if (m_addonControl &&
      static_cast<int>(m_renderRegion.x1) <
          gfxContect.GetWidth() &&
      static_cast<int>(m_renderRegion.y1) <
          gfxContect.GetHeight() &&
      m_renderRegion.x2 >= 0.0f &&
      m_renderRegion.y2 >=
          0.0f) // Do not render if control is outside of visible view (brings a badly fullscreen of a small control)
  {
    ...
    // a viewport??
    gfxContect.SetViewPort(m_posX, m_posY, m_width, m_height);
    gfxContect.CaptureStateBlock();
    m_addonControl->Render();
    gfxContect.ApplyStateBlock();
    gfxContect.RestoreViewPort();
  }
...
void CGUIWebAddonControl::FreeResources(bool immediately)
{
  ...
  if (strlen(m_props.strName) != 0)
  {
    auto& gfxContect = CServiceBroker::GetWinSystem()->GetGfxContext();

    // Destroy the addon control side only partly with the "false" to allow
    // background work.
    gfxContect.CaptureStateBlock(); //TODO locking
...
void CGUIWebAddonControl::SetFullscreen(bool enable)
{
  ...
  if (enable && !m_isFullScreen)
  {
    ...
    m_props.fXPos = 0;
    m_props.fYPos = 0;
    m_props.fWidth = gfxContect.GetWidth();
    m_props.fHeight = gfxContect.GetHeight();
    m_props.fSkinXPos = 0;
    m_props.fSkinYPos = 0;
  ...
  m_addonControl->ScreenSizeChange(m_props.fXPos, m_props.fYPos, m_props.fWidth, m_props.fHeight,
                                   m_isFullScreen);
}
//@}
...
bool CGUIWebAddonControl::GetHistory(std::vector<std::string>& historyWebsiteNames,
                                     bool behindCurrent)
{
  if (!m_addonControl)
    return false;
...
void CGUIWebAddonControl::SearchText(const std::string& text,
                                     bool forward,
                                     bool matchCase,
                                     bool findNext)
{
