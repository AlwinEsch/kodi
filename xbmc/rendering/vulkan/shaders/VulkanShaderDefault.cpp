...
constexpr const char* kVertexShaderFile = "default_vert.spv";
constexpr const char* kFragShaderFile = "default_frag.spv";

} // namespace

CVulkanShaderDefault::CVulkanShaderDefault(const VulkanData* vkData,
                                           CVulkanDeviceQueue* deviceQueue)
  : IVulkanShader(vkData, deviceQueue)
{
}

bool CVulkanShaderDefault::CreatePipelineLayout()
{
  ...
  VK_CHECK_RESULT(vkCreatePipelineLayout(m_vkData->vkDevice, &info, nullptr,
                                         &m_vkPipelineLayout[DEFAULT_TYPE_NO_BLEND]),
                  false);
  ...
  VK_CHECK_RESULT(vkCreatePipelineLayout(m_vkData->vkDevice, &info, nullptr,
                                         &m_vkPipelineLayout[DEFAULT_TYPE_BLEND]),
                  false);
  return true;
}

void CVulkanShaderDefault::DestroyPipelineLayout()
{
...
bool CVulkanShaderDefault::CreatePipeline()
{
  //================================================================================================
  ...
  {
    CLog::Log(LOGERROR, "Vulkan: Failed to load shaders: {0}, {1} ({2}:{3})",
              kVertexShaderFile, kFragShaderFile, __FILENAME__, __LINE__);
    return false;
  }
  ...
  pipelineCreateInfo.layout = m_vkPipelineLayout[DEFAULT_TYPE_BLEND];
  VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_vkData->vkDevice, m_vkData->vkPipelineCache, 1,
                                            &pipelineCreateInfo, nullptr,
                                            &m_vkPipeline[DEFAULT_TYPE_BLEND]),
                  false);

  m_vkPipelineLayout[DEFAULT_TYPE_NO_BLEND] = m_vkPipelineLayout[DEFAULT_TYPE_BLEND];
  //blendAttachmentState.blendEnable = VK_FALSE;

  //shaderStages[1] = fragShader;
  //pipelineCreateInfo.layout = m_vkPipelineLayout[DEFAULT_TYPE_NO_BLEND];
  //VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_vkData->vkDevice, m_vkData->vkPipelineCache, 1,
  //                                          &pipelineCreateInfo, nullptr,
  //                                          &m_vkPipeline[DEFAULT_TYPE_NO_BLEND]),
  //                false);
...
void CVulkanShaderDefault::DestroyPipeline()
{
...
bool CVulkanShaderDefault::CreateUniformBuffers()
{
...
void CVulkanShaderDefault::DestroyUniformBuffers()
{
  for (auto& entry : m_uniformBuffers)
  {
    if (entry.mapped)
...
void CVulkanShaderDefault::UpdateUniformBuffer(uint32_t index, const Uniform& uniformData)
{
...
//
//  VK_CHECK_RESULT(
//      vkCreatePipelineLayout(m_vkData->vkDevice, &info, nullptr, &m_vkPipelineLayout[TYPE_BLEND]),
//      false);
//
//  VK_CHECK_RESULT(vkCreatePipelineLayout(m_vkData->vkDevice, &info, nullptr,
//                                         &m_vkPipelineLayout[TYPE_NO_BLEND]),
//                  false);
//
//  return true;
//}
...
//  if (vertShader.module == VK_NULL_HANDLE || fragShader.module == VK_NULL_HANDLE)
//  {
//    CLog::Log(LOGERROR, "Vulkan: Failed to load shaders: {0}, {1} ({2}:{3})", kVertexShaderFile,
//              kFragShaderFile, __FILENAME__, __LINE__);
//    return false;
//  }
//
//  fragShader.pSpecializationInfo = &specializationInfo;
...
//
//  shaderStages[1] = fragShader;
//  pipelineCreateInfo.layout = m_vkPipelineLayout[TYPE_BLEND];
//  VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_vkData->vkDevice, m_vkData->vkPipelineCache, 1,
//                                            &pipelineCreateInfo, nullptr,
//                                            &m_vkPipeline[TYPE_BLEND]),
//                  false);
//
//  //blendAttachmentState.blendEnable = VK_FALSE;
//
//  shaderStages[1] = fragShader;
//  pipelineCreateInfo.layout = m_vkPipelineLayout[TYPE_NO_BLEND];
//  VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_vkData->vkDevice, m_vkData->vkPipelineCache, 1,
//                                            &pipelineCreateInfo, nullptr,
//                                            &m_vkPipeline[TYPE_NO_BLEND]),
//                  false);
//
//  // Shader modules are no longer needed once the graphics m_pipeline has been created
//  UnloadShader(vertShader);
...
//    VK_CHECK_RESULT(vkBindBufferMemory(m_vkData->vkDevice, entry.buffer, entry.memory, 0), false);
//    VK_CHECK_RESULT(
//        vkMapMemory(m_vkData->vkDevice, entry.memory, 0, sizeof(Uniform), 0, (void**)&entry.mapped),
//        false);
//
//    VK_CHECK_RESULT(vkAllocateDescriptorSets(m_vkData->vkDevice, &allocInfo, &entry.descriptorSet),
//                    false);
