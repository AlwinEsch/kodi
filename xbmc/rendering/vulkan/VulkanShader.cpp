...
CVulkanShader::CVulkanShader(const char* shader, const std::string& prefix)
{
  ...
  //VertexShader()->LoadSource("vulkan_shader.vert");
...
bool CVulkanShader::OnEnabled()
{
  ...
  //const GLfloat *projMatrix = vulkanMatrixProject.Get();
  //const GLfloat* modelMatrix = vulkanMatrixModview.Get();
  ...
  // * OpenGL, because CVulkanRenderSystem::SetScissors flips the Y axis. But all
  ...
  //// premultiplied-alpha textures (COverlayTextureVulkan) overrides this to 1.0
