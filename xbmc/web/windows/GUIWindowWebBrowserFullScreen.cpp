...
void CGUIWindowWebBrowserFullScreen::OnInitWindow(void)
{
  ...
  {
    m_webControl =
        new CGUIWebAddonControl(GetID(), CONTROL_WEBADDON, 0, 0, GetWidth(), GetHeight());
  }
...
bool CGUIWindowWebBrowserFullScreen::OnAction(const CAction& action)
{
  if (m_webControl)
  {
    const auto& locStr = CServiceBroker::GetResourcesComponent().GetLocalizeStrings();
    ...
    switch (id)
    {
      ...
      case ACTION_MOUSE_RIGHT_CLICK:
      {
        ...
        if (m_webControl->CanGoBack())
          choices.Add(CONTEXT_MENU_GO_BACK, locStr.Get(915));
        if (m_webControl->CanGoForward())
          choices.Add(CONTEXT_MENU_GO_FWD, locStr.Get(916));
        choices.Add(CONTEXT_MENU_OPEN_URL, locStr.Get(1051));
        choices.Add(CONTEXT_MENU_RELOAD, locStr.Get(914));
        choices.Add(CONTEXT_MENU_OPEN_FAVOURITES, locStr.Get(1036));
        choices.Add(CONTEXT_MENU_HOME, locStr.Get(913));
        choices.Add(CONTEXT_MENU_CLOSE_FULLSCREEN, locStr.Get(917));
        ...
        switch (choice)
        {
          case CONTEXT_MENU_GO_BACK:
            CServiceBroker::GetGUI()->GetWindowManager().SendMessage(GUI_MSG_WEB_CONTROL_GO_BACK,
                                                                     GetID(), CONTROL_WEBADDON);
            break;
          case CONTEXT_MENU_GO_FWD:
            CServiceBroker::GetGUI()->GetWindowManager().SendMessage(GUI_MSG_WEB_CONTROL_GO_FWD,
                                                                     GetID(), CONTROL_WEBADDON);
            break;
          case CONTEXT_MENU_OPEN_URL:
          {
            std::string url = m_webControl->OpenedAddress();
            if (CGUIKeyboardFactory::ShowAndGetInput(url, CVariant{locStr.Get(912)},
                                                     true) &&
                !url.empty())
            {
          ...
          case CONTEXT_MENU_RELOAD:
            CServiceBroker::GetGUI()->GetWindowManager().SendMessage(GUI_MSG_WEB_CONTROL_RELOAD,
                                                                     GetID(), CONTROL_WEBADDON);
            break;
          ...
          case CONTEXT_MENU_HOME:
            CServiceBroker::GetGUI()->GetWindowManager().SendMessage(GUI_MSG_WEB_CONTROL_HOME,
                                                                     GetID(), CONTROL_WEBADDON);
            break;
