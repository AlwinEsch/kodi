  ...
  switch (id)
  {
    ...
    case ACTION_MOUSE_RIGHT_CLICK:
    {
      ...
      else if (focusedControl == CONTROL_LIST_ACTIVE_WEBSITE_TABS)
      {
        ...
        CContextButtons choices;
        choices.Add(TABS_CONTEXT_DELETE_TAB, CServiceBroker::GetResourcesComponent().GetLocalizeStrings().Get(920));
        if (m_selectedItem == selected && m_controls.size() > 1)
          choices.Add(TABS_CONTEXT_DELETE_OTHER_TABS, CServiceBroker::GetResourcesComponent().GetLocalizeStrings().Get(921));
        choices.Add(TABS_CONTEXT_ADD_TO_FAVORITES, CServiceBroker::GetResourcesComponent().GetLocalizeStrings().Get(922));
        int choice = CGUIDialogContextMenu::ShowAndGetChoice(choices);
        if (choice < 0)
          break;
...
bool CGUIWindowWebBrowser::ControlCreate(const std::string& url)
{
  ...
  itemSiteNew->SetLabel(CServiceBroker::GetResourcesComponent().GetLocalizeStrings().Get(919));
