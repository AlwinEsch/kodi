...
bool CWebAddon::DestroyControl(const WEB_HANDLE handle, bool complete)
{
  return m_struct.toAddon->destroy_control(&m_struct, handle, complete);
}

void CWebAddon::Render(const WEB_HANDLE handle)
{
  m_struct.toAddon->control_render(&m_struct, handle);
}

bool CWebAddon::OpenWebsite(const WEB_HANDLE handle, const std::string& url)
{
  return m_struct.toAddon->control_open_website(&m_struct, handle, url.c_str());
}

void CWebAddon::Reload(const WEB_HANDLE handle)
{
  m_struct.toAddon->control_web_cmd_reload(&m_struct, handle);
}

void CWebAddon::StopLoad(const WEB_HANDLE handle)
{
  m_struct.toAddon->control_web_cmd_stop_load(&m_struct, handle);
}

void CWebAddon::GoBack(const WEB_HANDLE handle)
{
  m_struct.toAddon->control_web_cmd_nav_back(&m_struct, handle);
}

void CWebAddon::GoForward(const WEB_HANDLE handle)
{
  m_struct.toAddon->control_web_cmd_nav_forward(&m_struct, handle);
}

void CWebAddon::OpenOwnContextMenu(const WEB_HANDLE handle)
{
  m_struct.toAddon->control_web_open_own_context_menu(&m_struct, handle);
}

bool CWebAddon::Dirty(const WEB_HANDLE handle)
{
  return m_struct.toAddon->control_dirty(&m_struct, handle);
}

bool CWebAddon::OnInit(const WEB_HANDLE handle)
{
  return m_struct.toAddon->control_on_init(&m_struct, handle);
}

bool CWebAddon::OnAction(const WEB_HANDLE handle, const addon_action_data* action, int& nextItem)
{
  return m_struct.toAddon->control_on_action(&m_struct, handle, action, &nextItem);
}

bool CWebAddon::OnMouseEvent(
    const WEB_HANDLE handle, int id, double x, double y, double offsetX, double offsetY, int state)
{
  return m_struct.toAddon->control_on_mouse_event(&m_struct, handle, id, x, y, offsetX, offsetY,
                                                  state);
}

bool CWebAddon::ControlGetHistory(const WEB_HANDLE handle,
...
void CWebAddon::ControlSearchText(
    const WEB_HANDLE handle, const std::string& text, bool forward, bool matchCase, bool findNext)
{
  m_struct.toAddon->control_search_text(&m_struct, handle, text.c_str(), forward, matchCase,
                                        findNext);
}

void CWebAddon::ControlStopSearch(const WEB_HANDLE handle, bool clearSelection)
{
  m_struct.toAddon->control_stop_search(&m_struct, handle, clearSelection);
}

void CWebAddon::ControlScreenSizeChange(
    const WEB_HANDLE handle, float x, float y, float width, float height, bool fullscreen)
{
