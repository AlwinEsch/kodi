...
void CWebManager::Init()
{
  ...
  CServiceBroker::GetJobManager()->Submit([this] {
    ...
    CServiceBroker::GetAddonMgr().Events().Subscribe(this, [this](const ADDON::AddonEvent& event) { OnEvent(event); });
    Deinit();
    UpdateAddons();
...
void CWebManager::UpdateAddons()
{
  ...
  CServiceBroker::GetAddonMgr().GetAddonInfos(addonInfos, false, AddonType::WEB_MANAGER);
  if (addonInfos.empty())
    return;
