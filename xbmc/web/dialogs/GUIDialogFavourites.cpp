...
std::string CGUIDialogWebFavourites::ShowAndGetFavourite(CWebDatabase* database)
{
  ...
  m_database = database;
  if (m_database->Open() && m_database->GetFavourites(entries))
  {
    for (const auto& entry : entries)
    {
      CFileItemPtr item(std::make_shared<CFileItem>(entry->GetName()));
      item->SetLabel2(entry->GetURL());
      item->SetArt("icon", entry->GetIcon());
      m_vecList->Add(item);
    }
    m_vecList->Sort(SortBy::LABEL, SortOrder::ASCENDING);
...
{
  CGUIDialogWebFavourites* dialog =
      CServiceBroker::GetGUI()->GetWindowManager().GetWindow<CGUIDialogWebFavourites>(
          WINDOW_DIALOG_WEB_FAVOURITES);
  if (dialog)
  {
...
bool CGUIDialogWebFavourites::OnMessage(CGUIMessage& message)
{
  switch (message.GetMessage())
  {
    ...
    {
      m_selectedItem = -1;
      SetHeading(CVariant{CServiceBroker::GetResourcesComponent().GetLocalizeStrings().Get(1036)});
      CGUIDialogBoxBase::OnMessage(message);
      return true;
    }
    break;
...
void CGUIDialogWebFavourites::OnInitWindow()
{
  ...
  SET_CONTROL_LABEL(
      ...
      StringUtils::Format(
          "%i %s", m_vecList->Size(),
          CServiceBroker::GetResourcesComponent().GetLocalizeStrings().Get(127).c_str()));
  SET_CONTROL_HIDDEN(CONTROL_EXTRA_BUTTON);
  SET_CONTROL_LABEL(CONTROL_CANCEL_BUTTON,
                    CServiceBroker::GetResourcesComponent().GetLocalizeStrings().Get(15067));
...
void CGUIDialogWebFavourites::OnPopupMenu(int item)
{
  ...
  else if (button == 2)
  {
    ...
    if (m_database->RemoveFavourite(m_vecList->Get(item)->GetLabel(),
                                    m_vecList->Get(item)->GetLabel2()))
    {
