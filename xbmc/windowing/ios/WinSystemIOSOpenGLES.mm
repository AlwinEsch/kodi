...
#include "WinSystemIOSOpenGLES.h"
...
void CWinSystemIOSOpenGLES::Register()
{
  KODI::WINDOWING::CWindowSystemFactory::RegisterWindowSystem(CreateWinSystem);
}

std::unique_ptr<CWinSystemBase> CWinSystemIOSOpenGLES::CreateWinSystem()
{
  return std::make_unique<CWinSystemIOSOpenGLES>();
}

int CWinSystemIOSOpenGLES::GetDisplayIndexFromSettings()
{
...
CWinSystemIOSOpenGLES::CWinSystemIOSOpenGLES() : CWinSystemBase()
{
...
CWinSystemIOSOpenGLES::~CWinSystemIOSOpenGLES()
{
  delete m_pDisplayLink;
}

bool CWinSystemIOSOpenGLES::InitWindowSystem()
{
	return CWinSystemBase::InitWindowSystem();
}

bool CWinSystemIOSOpenGLES::DestroyWindowSystem()
{
  return true;
}

bool CWinSystemIOSOpenGLES::CreateNewWindow(const std::string& name, bool fullScreen, RESOLUTION_INFO& res)
{
...
bool CWinSystemIOSOpenGLES::DestroyWindow()
{
  return true;
}

bool CWinSystemIOSOpenGLES::ResizeWindow(int newWidth, int newHeight, int newLeft, int newTop)
{
  //NSLog(@"%s", __PRETTY_FUNCTION__);
...
bool CWinSystemIOSOpenGLES::SetFullScreen(bool fullScreen, RESOLUTION_INFO& res, bool blankOtherDisplays)
{
  //NSLog(@"%s", __PRETTY_FUNCTION__);
...
bool CWinSystemIOSOpenGLES::SwitchToVideoMode(int width, int height, double refreshrate)
{
  bool ret = false;
...
bool CWinSystemIOSOpenGLES::GetScreenResolution(int* w, int* h, double* fps, int screenIdx)
{
...
void CWinSystemIOSOpenGLES::UpdateResolutions()
{
...
void CWinSystemIOSOpenGLES::FillInVideoModes(int screenIdx)
{
...
bool CWinSystemIOSOpenGLES::IsExtSupported(const char* extension) const
{
...
bool CWinSystemIOSOpenGLES::BeginRender()
{
  bool rtn;
...
bool CWinSystemIOSOpenGLES::EndRender()
{
  bool rtn;
...
void CWinSystemIOSOpenGLES::Register(IDispResource *resource)
{
  std::unique_lock lock(m_resourceSection);
  m_resources.push_back(resource);
}

void CWinSystemIOSOpenGLES::Unregister(IDispResource* resource)
{
  std::unique_lock lock(m_resourceSection);
...
void CWinSystemIOSOpenGLES::OnAppFocusChange(bool focus)
{
  std::unique_lock lock(m_resourceSection);
  m_bIsBackgrounded = !focus;
  CLog::Log(LOGDEBUG, "CWinSystemIOSOpenGLES::OnAppFocusChange: {}", focus ? 1 : 0);
...
bool CWinSystemIOSOpenGLES::InitDisplayLink(CVideoSyncIos *syncImpl)
{
...
void CWinSystemIOSOpenGLES::DeinitDisplayLink(void)
{
...
void CWinSystemIOSOpenGLES::PresentRenderImpl(bool rendered)
{
  //glFlush;
  if (rendered)
...
bool CWinSystemIOSOpenGLES::HasCursor()
{
  // apple touch devices
  return false;
}

void CWinSystemIOSOpenGLES::NotifyAppActiveChange(bool bActivated)
{
...
bool CWinSystemIOSOpenGLES::Minimize()
{
...
bool CWinSystemIOSOpenGLES::Restore()
{
  return false;
}

bool CWinSystemIOSOpenGLES::Hide()
{
  return true;
}

bool CWinSystemIOSOpenGLES::Show(bool raise)
{
  return true;
}

CVEAGLContext CWinSystemIOSOpenGLES::GetEAGLContextObj()
{
  return [g_xbmcController getEAGLContextObj];
}

std::vector<std::string> CWinSystemIOSOpenGLES::GetConnectedOutputs()
{
...
void CWinSystemIOSOpenGLES::MoveToTouchscreen()
{
  CDisplaySettings::GetInstance().SetMonitor(CONST_TOUCHSCREEN);
}

std::unique_ptr<CVideoSync> CWinSystemIOSOpenGLES::GetVideoSync(CVideoReferenceClock* clock)
{
  std::unique_ptr<CVideoSync> pVSync(new CVideoSyncIos(clock, *this));
  return pVSync;
}

bool CWinSystemIOSOpenGLES::MessagePump()
{
