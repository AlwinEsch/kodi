...
#include "WinSystemX11VulkanContext.h"
...
#include "cores/RetroPlayer/rendering/VideoRenderers/RPRendererOpenVulkan.h"
...
void CWinSystemX11VulkanContext::Register()
{
  KODI::WINDOWING::CWindowSystemFactory::RegisterWindowSystem(CreateWinSystem, "x11");
}

std::unique_ptr<CWinSystemBase> CWinSystemX11VulkanContext::CreateWinSystem()
{
  return std::make_unique<CWinSystemX11VulkanContext>();
}

CWinSystemX11VulkanContext::~CWinSystemX11VulkanContext()
{
  delete m_pGLContext;
}

void CWinSystemX11VulkanContext::PresentRenderImpl(bool rendered)
{
...
void CWinSystemX11VulkanContext::SetVSyncImpl(bool enable)
{
  m_pGLContext->SetVSync(enable);
}

bool CWinSystemX11VulkanContext::IsExtSupported(const char* extension) const
{
  if (strncmp(extension, m_pGLContext->ExtPrefix().c_str(), 4) != 0)
    return CRenderSystemVulkan::IsExtSupported(extension);

  return m_pGLContext->IsExtSupported(extension);
}

EGLDisplay CWinSystemX11VulkanContext::GetEGLDisplay() const
{
  return m_pGLContext->m_eglDisplay;
}

EGLSurface CWinSystemX11VulkanContext::GetEGLSurface() const
{
  return m_pGLContext->m_eglSurface;
}

EGLContext CWinSystemX11VulkanContext::GetEGLContext() const
{
  return m_pGLContext->m_eglContext;
}

EGLConfig CWinSystemX11VulkanContext::GetEGLConfig() const
{
  return m_pGLContext->m_eglConfig;
}

bool CWinSystemX11VulkanContext::BindTextureUploadContext()
{
  if (m_pGLContext)
...
bool CWinSystemX11VulkanContext::UnbindTextureUploadContext()
{
  if (m_pGLContext)
...
bool CWinSystemX11VulkanContext::HasContext()
{
  if (m_pGLContext)
...
bool CWinSystemX11VulkanContext::SetWindow(int width, int height, bool fullscreen, const std::string& output, int* winstate)
{
  int newwin = 0;
...
bool CWinSystemX11VulkanContext::CreateNewWindow(const std::string& name, bool fullScreen, RESOLUTION_INFO& res)
{
  CLog::Log(LOGINFO, "CWinSystemX11VulkanContext::CreateNewWindow");
...
bool CWinSystemX11VulkanContext::ResizeWindow(int newWidth, int newHeight, int newLeft, int newTop)
{
  m_newGlContext = false;
  CWinSystemX11::ResizeWindow(newWidth, newHeight, newLeft, newTop);
  CRenderSystemVulkan::ResetRenderSystem(newWidth, newHeight);
...
void CWinSystemX11VulkanContext::FinishWindowResize(int newWidth, int newHeight)
{
  m_newGlContext = false;
...
bool CWinSystemX11VulkanContext::SetFullScreen(bool fullScreen, RESOLUTION_INFO& res, bool blankOtherDisplays)
{
  m_newGlContext = false;
  CWinSystemX11::SetFullScreen(fullScreen, res, blankOtherDisplays);
  CRenderSystemVulkan::ResetRenderSystem(res.iWidth, res.iHeight);
...
bool CWinSystemX11VulkanContext::DestroyWindowSystem()
{
  if (m_pGLContext)
...
bool CWinSystemX11VulkanContext::DestroyWindow()
{
  if (m_pGLContext)
    m_pGLContext->Detach();
...
XVisualInfo* CWinSystemX11VulkanContext::GetVisual()
{
...
bool CWinSystemX11VulkanContext::RefreshGLContext(bool force)
{
  bool success = false;
  if (m_pGLContext)
  {
