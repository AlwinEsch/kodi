...
#include "WinSystemAndroidVulkanContext.h"
...
void CWinSystemAndroidVulkanContext::Register()
{
  KODI::WINDOWING::CWindowSystemFactory::RegisterWindowSystem(CreateWinSystem);
}

std::unique_ptr<CWinSystemBase> CWinSystemAndroidVulkanContext::CreateWinSystem()
{
  return std::make_unique<CWinSystemAndroidVulkanContext>();
}

bool CWinSystemAndroidVulkanContext::InitWindowSystem()
{
  ...
  //CLog::Log(LOGDEBUG,
  //          "CWinSystemAndroidVulkanContext::InitWindowSystem: HDRConfig: {}, HDRExtensions: {}",
...
bool CWinSystemAndroidVulkanContext::CreateNewWindow(const std::string& name,
                                               bool fullScreen,
                                               RESOLUTION_INFO& res)
{
...
bool CWinSystemAndroidVulkanContext::ResizeWindow(int newWidth, int newHeight, int newLeft, int newTop)
{
  CRenderSystemGLES::ResetRenderSystem(newWidth, newHeight);
  return true;
}

bool CWinSystemAndroidVulkanContext::SetFullScreen(bool fullScreen, RESOLUTION_INFO& res, bool blankOtherDisplays)
{
...
void CWinSystemAndroidVulkanContext::SetVSyncImpl(bool enable)
{
  // We use Choreographer for timing
  //m_pGLContext.SetVSync(false);
}

void CWinSystemAndroidVulkanContext::PresentRenderImpl(bool rendered)
{
  //if (!m_nativeWindow)
  //{
  //  usleep(10000);
...
float CWinSystemAndroidVulkanContext::GetFrameLatencyAdjustment()
{
  return CXBMCApp::Get().GetFrameLatencyMs();
}

//EGLDisplay CWinSystemAndroidVulkanContext::GetEGLDisplay() const
//{
//  return m_pGLContext.GetEGLDisplay();
//}
//
//EGLSurface CWinSystemAndroidVulkanContext::GetEGLSurface() const
//{
//  return m_pGLContext.GetEGLSurface();
//}
//
//EGLContext CWinSystemAndroidVulkanContext::GetEGLContext() const
//{
//  return m_pGLContext.GetEGLContext();
//}
//
//EGLConfig  CWinSystemAndroidVulkanContext::GetEGLConfig() const
//{
//  return m_pGLContext.GetEGLConfig();
//}

std::unique_ptr<CVideoSync> CWinSystemAndroidVulkanContext::GetVideoSync(CVideoReferenceClock* clock)
{
  std::unique_ptr<CVideoSync> pVSync(new CVideoSyncAndroid(clock));
  return pVSync;
}

bool CWinSystemAndroidVulkanContext::CreateSurface()
{
...
bool CWinSystemAndroidVulkanContext::IsHDRDisplay()
{
...
bool CWinSystemAndroidVulkanContext::SetHDR(const VideoPicture* videoPicture)
{
...
//    {
//      CLog::Log(LOGDEBUG, "CWinSystemAndroidVulkanContext::SetHDR: ColorSpace: {}", HDRColorSpace);
//
