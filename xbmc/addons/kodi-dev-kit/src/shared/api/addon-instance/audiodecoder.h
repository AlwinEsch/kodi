...
typedef struct KODI_ADDON_AUDIODECODER_FUNC_V1_TO_V1
{
  ...
  PFN_KODI_ADDON_AUDIODECODER_SUPPORTS_FILE_V1 supports_file; /*---AUTO<EQUAL_FROM_TO:1_1>---*/
...
typedef bool(ATTR_INT_APIENTRYP PFN_INT_KODI_ADDON_AUDIODECODER_SUPPORTS_FILE_V1)(
...
struct directFuncToAddon_addoninstance_audiodecoder_h
{
  ...
  PFN_INT_KODI_ADDON_AUDIODECODER_SUPPORTS_FILE_V1 kodi_addon_audiodecoder_supports_file_v1;
...
struct IFC_KODI_ADDON_AUDIODECODER_INFO_TAG
{
  IFC_KODI_ADDON_AUDIODECODER_INFO_TAG() = default;
  IFC_KODI_ADDON_AUDIODECODER_INFO_TAG(const KODI_ADDON_AUDIODECODER_INFO_TAG* c_data)
  {
    if (c_data == nullptr)
      return;
    ...
    if (c_data->cover_art_path)
      cover_art_path = c_data->cover_art_path;
    if (c_data->cover_art_mem_mimetype)
      cover_art_mem_mimetype = c_data->cover_art_mem_mimetype;
    if (c_data->cover_art_mem)
      cover_art_mem.assign(c_data->cover_art_mem,
                           c_data->cover_art_mem + c_data->cover_art_mem_size);
    cover_art_mem_size = c_data->cover_art_mem_size;
  }

  static inline void CleanCStructure(KODI_ADDON_AUDIODECODER_INFO_TAG* c_data)
  {
    if (c_data == nullptr)
      return;
    ...
    if (c_data->cover_art_path)
    {
      free(c_data->cover_art_path);
      c_data->cover_art_path = nullptr;
    }
    if (c_data->cover_art_mem_mimetype)
    {
      free(c_data->cover_art_mem_mimetype);
      c_data->cover_art_mem_mimetype = nullptr;
    }
    if (c_data->cover_art_mem)
    {
      free(c_data->cover_art_mem);
      c_data->cover_art_mem = nullptr;
    }
  }

  void SetCStructure(KODI_ADDON_AUDIODECODER_INFO_TAG* c_data)
  {
    if (c_data == nullptr)
      return;
    ...
    if (!cover_art_mem.empty())
    {
      size_t size = cover_art_mem.size();
      c_data->cover_art_mem = static_cast<uint8_t*>(malloc(sizeof(uint8_t) * size));
      memcpy(c_data->cover_art_mem, cover_art_mem.data(), size);
    }
  ...
  std::vector<uint8_t> cover_art_mem;
...
{
  funcChild_kodi_addon_audiodecoder_supports_file_v1,
...
typedef std::tuple<PtrValue, std::string> msgChild__IN_kodi_addon_audiodecoder_supports_file_v1; /* Autogenerated */
typedef std::tuple<bool> msgChild_OUT_kodi_addon_audiodecoder_supports_file_v1; /* Autogenerated */
...
typedef std::tuple<bool, IFC_KODI_ADDON_AUDIODECODER_INFO_TAG> msgChild_OUT_kodi_addon_audiodecoder_read_tag_v1; /* Autogenerated */
