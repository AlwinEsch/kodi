    ...
    directFuncToAddon_addoninstance_audiodecoder_h* ifcToAddon)
{
  ...
  ifcToAddon->kodi_addon_audiodecoder_supports_file_v1 = kodi_addon_audiodecoder_supports_file_v1;
                                                           ...
                                                           msgpack::sbuffer& out)
{
  ...
  switch (func)
  {
    case funcChild_kodi_addon_audiodecoder_supports_file_v1:
    {
      ...
      msgChild__IN_kodi_addon_audiodecoder_supports_file_v1 t = in.get().as<decltype(t)>();
      KODI_ADDON_HDL auto_gen_hdl = reinterpret_cast<KODI_ADDON_HDL>(std::get<0>(t));
      const std::string& file = std::get<1>(t);
      bool auto_gen_ret =
          kodi_addon_audiodecoder_supports_file_v1(this, auto_gen_hdl, file.c_str());

      msgpack::pack(out, msgChild_OUT_kodi_addon_audiodecoder_supports_file_v1(auto_gen_ret));
      return true;
    }
    ...
    case funcChild_kodi_addon_audiodecoder_read_tag_v1:
    {
      ...
      IFC_KODI_ADDON_AUDIODECODER_INFO_TAG::CleanCStructure(&tag);
      return true;
    }
    ...
    void* thisClassHdl, const KODI_ADDON_AUDIODECODER_HDL hdl, const char* file) // Added with API 1
{
  ...
  if (union_data->types.kodi_addon_audiodecoder.v1.supports_file)
  {
    auto_gen_ret =
        union_data->types.kodi_addon_audiodecoder.v1.supports_file(union_data->hdl, file);
  }
  return auto_gen_ret;
}
