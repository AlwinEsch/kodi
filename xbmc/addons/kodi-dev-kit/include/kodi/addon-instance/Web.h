...
class ATTR_DLL_LOCAL WebKeyboardInputEvent : public CStructHdl<WebKeyboardInputEvent, WEB_KEYBOARD_INPUT_EVENT>
{
  /*! \cond PRIVATE */
...
public:
  ...
  ///
  CInstanceWeb() : IAddonInstance(IInstanceInfo(CPrivateBase::m_interface->firstKodiInstance))
  {
  ...
  explicit CInstanceWeb(const IInstanceInfo& instance) : IAddonInstance(instance)
  {
    if (CPrivateBase::m_interface->globalSingleInstance != nullptr)
      throw std::logic_error("kodi::addon::CInstanceWeb: Creation of multiple together with "
