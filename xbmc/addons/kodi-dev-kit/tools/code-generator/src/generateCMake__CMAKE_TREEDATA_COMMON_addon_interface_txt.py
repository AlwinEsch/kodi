...
def GenerateCMake__CMAKE_TREEDATA_COMMON_addon_interface_txt_RelatedCheck(filename):
    """
    This function is called by git update to be able to assign changed files to the dev kit.
    """
    return True if filename == "cmake/treedata/common/addon_interface.txt" else False


def GenerateCMake__CMAKE_TREEDATA_COMMON_addon_interface_txt(options):
    """
    This function generate the "cmake/treedata/common/addon_interface.txt"
    by scan of related directories to use for add-on interface build.
    """
    gen_file = "cmake/treedata/common/addon_interface.txt"

    Log.PrintBegin("Check for {}".format(gen_file))

    scan_dir = "{}{}/**/CMakeLists.txt".format(KODI_DIR, INTERFACE_DIR)
    ...
    for entry in glob.glob(scan_dir, recursive=True):
        ...
        with open(entry) as search:
            for line in search:
                ...
                m = re.search("^ *core_add_library\((.*)\)", line)
                if m:
