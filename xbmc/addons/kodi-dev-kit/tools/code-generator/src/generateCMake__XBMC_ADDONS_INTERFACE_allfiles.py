...
def GenerateCMake__XBMC_ADDONS_INTERFACE_all_files_RelatedCheck(filename):
    """
    This function is called by git update to be able to assign changed files to the dev kit.
    """
    scan_dir = "{}{}/**/".format(KODI_DIR, INTERFACE_DIR)
...
def GenerateCMake__XBMC_ADDONS_INTERFACE_all_files(options):
    """
    This function generate the "CMakeLists.txt" in xbmc/addons/interface
    and sub dirs by scan of available files
    """
    Log.PrintBegin("Generate CMakeLists.txt files in {} dirs".format(INTERFACE_DIR))
