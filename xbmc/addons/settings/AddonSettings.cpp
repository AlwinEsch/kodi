...
namespace ADDON
{
CAddonSettings::CAddonSettings(const std::shared_ptr<const IAddon>& addon, uint32_t instanceId)
  : CSettingsBase(),
    m_addonId(addon->ID()),
    ...
    m_logger(CServiceBroker::GetLogging().GetLogger(
        StringUtils::Format("CAddonSettings[{}@{}]", instanceId, m_addonId)))
{ }
...
{
  if (GetSetting(ADDON_SETTING_ADDON_INSTANCE_NAME_VALUE) ||
      GetSetting(ADDON_SETTING_ADDON_INSTANCE_ENABLED_VALUE))
  {
    CLog::Log(
        LOGDEBUG,
  ...
  auto itr = std::find_if(groups.begin(), groups.end(),
                          [](SettingGroupPtr group)
                          { return group->GetId() == ADDON_SETTING_ADDON_INSTANCE_GROUP; });
  ...
  {
    group = std::make_shared<CSettingGroup>(ADDON_SETTING_ADDON_INSTANCE_GROUP, mgr);
    group->SetLabel(10017);
    category->AddGroupToBegin(group);
  }

  const std::shared_ptr<CSettingString> name =
      std::make_shared<CSettingString>(ADDON_SETTING_ADDON_INSTANCE_NAME_VALUE, 10019, "", mgr);
  name->SetAllowEmpty(false);
  name->SetControl(std::make_shared<CSettingControlEdit>());
  if (!mgr->AddSetting(name, section, category, group))
    return false;

  const std::shared_ptr<CSettingBool> enabled =
      std::make_shared<CSettingBool>(ADDON_SETTING_ADDON_INSTANCE_ENABLED_VALUE, 10020, true, mgr);
